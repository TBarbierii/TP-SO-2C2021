...
void crear_archivos_swap(t_list* archivos_swap, int cantidad_particiones, t_log* logger) {
    ...
    while(!list_is_empty(archivos_swap)) {
        ...
        log_info(logger,"Se crea el archivo SWAP en el path: %s, de size: %d cantidad de frames disponibles: %i y id: %i",nuevoArchivo->path, tamanio_swap, cantidad_frames_disponibles(nuevoArchivo), nuevoArchivo->id);
